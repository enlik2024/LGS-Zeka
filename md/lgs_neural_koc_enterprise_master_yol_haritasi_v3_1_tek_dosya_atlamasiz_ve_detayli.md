...
Bu proje faz-kapılı bir master sözleşmeye bağlıdır. AI IDE aşağıdaki dosya/konum düzenini **yorumlamadan** korumalıdır.

### Kanonik dosyalar
Aşağıdaki dosyalar bu projede “tek doğru isim/konum” kabul edilir:
...
- `app/content_ingest_engine.py` *(FAZ D-2 sonrası aktif)*
- `config/settings.yaml`
- `config/feature_flags.yaml`
- `tests/`

### Kilit kuralları
1) Bu dosyaların **adı değiştirilemez**.  
...
4) Yeni dosya eklemek gerekiyorsa sadece şu dizinlere eklenebilir:  
   ...
   - `tests/`  
   - `config/`  
5) Faz D-2 tamamlanmadan `content_ingest_engine.py` için PDF otomasyonu geliştirilmez.
